...
class CustomDataset(MocapDataset):
    def __init__(self, detections_path, remove_static_joints=True):
        ...
        #Load serialized dataset (2D locations)
        ...
        for video_name, res in resolutions.items(): #will only have one item if only one video, which will look like {'output.mp4': {'w': 1080, 'h': 1920}}
            cam = {}

            #add in custom camera params
            ...
            #add video name and resolution data taken from the loaded npz
            cam['id'] = video_name
            cam['res_w'] = res['w']
            cam['res_h'] = res['h']
        ...
        if remove_static_joints:
            print("Remove_static_joints true")
            ...
            print("NOT remove_static_joints")
